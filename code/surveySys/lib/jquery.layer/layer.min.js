    ...
    callback([
        ...
        '<div class="'+ doms[0] +' '+ (doms.anim[config.shift]||'') + (' layui-layer-'+ready.type[config.type]) + (((config.type == 0 || config.type == 2) && !config.shade) ? ' layui-layer-border' : '') + ' ' + (config.skin||'') +'" id="'+ doms[0] + times +'" type="'+ ready.type[config.type] +'" times="'+ times +'" showtime="'+ config.time +'" conType="'+ (conType ? 'object' : 'string') +'" style="z-index: '+ zIndex +'; width:'+ config.area[0] + ';height:' + config.area[1] + (config.fix ? '' : ';position:absolute;') +'">'
            ...
            + '<span class="layui-layer-setwin">'+ function(){
                ...
                config.closeBtn && (closebtn += '<a class="layui-layer-ico '+ doms[7] +' '+ doms[7] + (config.title ? config.closeBtn : (config.type == 4 ? '1' : '2')) +'" href="javascript:;"><i class="vvicon">&#xe60a;</i></a>');
                return closebtn;
            }() + '</span>'
            + (config.btn ? function(){
                var button = '';
                ...
                for(var i = 0, len = config.btn.length; i < len; i++){
                    var _className =  doms[6],
                        _i = i;
                    if(len==1){
                        _i = 1;
                        _className = _className + '1' + ' full';
                    }else{
                        _className += (''+ i)
                    }
                    var _target = config.target ? config.target[i] : '';
                    if(_target=='_blank'){
                        button += '<a data-index="'+ _i +'" class="'+ _className +'" target="_blank" class="link" href="'+config.src[i]+'">'+ config.btn[i] +'</a>'
                    }else{
                        button += '<a data-index="'+ _i +'" class="'+ _className +'">'+ config.btn[i] +'</a>'
                    }
                }
